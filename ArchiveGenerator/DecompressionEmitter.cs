...
namespace ArchiveGenerator
{
	public static class DecompressionEmitter
	{
		...
		private static MethodBuilder EmitDecompressLZF(TypeBuilder parent)
		{
			...
			const int L_strm = 0;
			gen.DeclareLocal(typeof(byte[])).SetLocalSymInfo("strm");
			const int L_iidx = 1;
			gen.DeclareLocal(typeof(uint)).SetLocalSymInfo("iidx");
			const int L_oidx = 2;
			gen.DeclareLocal(typeof(uint)).SetLocalSymInfo("oidx");
			const int L_ctrl = 3;
			gen.DeclareLocal(typeof(uint)).SetLocalSymInfo("ctrl");
			const int L_len = 4;
			gen.DeclareLocal(typeof(uint)).SetLocalSymInfo("len");
			const int L_reference = 5;
			gen.DeclareLocal(typeof(int)).SetLocalSymInfo("reference");
